...
export function notify(error: Error, options: NotifyOptions = {}): void {
  ...
  const mergedContext = sanitize({ ...globalContext, ...localContext, ...options.context })
  const mergedUser = sanitize({ ...globalUser, ...localUser, ...options.user })
  const mergedRequest = sanitize({ ...localRequest, ...options.request })
